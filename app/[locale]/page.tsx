  ...
  return (
    <div className="min-h-[100dvh] bg-primary overflow-x-hidden">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              <a href="#testimonials" className="text-sm text-text-secondary hover:text-text-primary transition-colors duration-300">{t('nav.testimonials')}</a>
      ...
      <div className={`fixed inset-0 z-40 bg-primary flex flex-col items-center justify-center p-8 transition-transform duration-300 ${mobileMenuOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <a href="#testimonials" onClick={() => setMobileMenuOpen(false)} className="text-2xl mb-4">{t('nav.testimonials')}</a>
      ...
      <section className="relative min-h-[100dvh] pt-28 md:pt-32">
        <div className="container mx-auto px-4 h-full">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 min-h-[calc(100dvh-120px)] items-center py-16 lg:py-0">
            <div className="lg:col-span-6 flex flex-col justify-center">
              ...
              <p className="mt-8 text-lg md:text-xl text-text-secondary max-w-md leading-relaxed fade-in fade-in-2">{t('common.tagline')}</p>
              <div className="mt-10 flex flex-col sm:flex-row gap-4 fade-in fade-in-3">
                <button onClick={() => scrollToBooking()} className="btn-primary">{t('hero.cta_primary')}<span className="arrow">→</span></button>
                <a href="#work" className="btn-secondary"><span>{t('hero.cta_secondary')}</span></a>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-16 md:mb-24 gap-6">
            <div>
              ...
              <h2 className="text-text-primary fade-in fade-in-1" style={{ fontSize: 'clamp(40px, 5vw, 72px)', lineHeight: '1.0' }}>{t('portfolio.subtitle')}</h2>
            </div>
            <a href="#work" className="btn-secondary btn-compact fade-in fade-in-2"><span>{t('portfolio.view_all')}</span></a>
          </div>
                ...
                return (
                  <div key={key} className="md:col-span-12 image-hover group cursor-pointer fade-in" style={{ animationDelay: `${0.1 * (index + 1)}s` }}>
                    <div className="relative aspect-[21/9] mb-4 overflow-hidden rounded-sm photo-placeholder">
                      ...
                      <div className="absolute top-6 left-6 z-10"><span className="eyebrow text-text-primary/70 bg-primary/50 px-2 py-1 rounded-sm">0{index + 1}</span></div>
                    </div>
                    <h3 className="text-text-primary group-hover:text-accent transition-colors" style={{ fontSize: 'clamp(24px, 2.5vw, 36px)', lineHeight: '1.1' }}>{t(`portfolio.categories.${key}`)}</h3>
                  </div>
                );
              }
              return (
                <div key={key} className={`image-hover group cursor-pointer fade-in ${isLarge ? 'md:col-span-7' : 'md:col-span-5'} ${index === 1 || index === 3 ? 'md:mt-24' : ''}`} style={{ animationDelay: `${0.1 * (index + 1)}s` }}>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            {serviceKeys.map((key, index) => {
              ...
              return (
                <div key={key} className={`pricing-card border border-line bg-primary p-8 md:p-10 fade-in ${isLast ? 'md:col-span-2' : ''}`} style={{ animationDelay: `${0.08 * (index + 1)}s` }}>
                  <div className="flex items-start justify-between mb-6 gap-4">
                    <div className="flex-1">
                      <span className="eyebrow text-text-muted mb-3 block">0{index + 1}</span>
                      <h3 className="text-text-primary font-serif" style={{ fontSize: 'clamp(24px, 2.5vw, 36px)', lineHeight: '1.1' }}>{t(`pricing.items.${key}.title`)}</h3>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="flex items-baseline gap-2">
                        <span className="text-accent font-serif" style={{ fontSize: 'clamp(32px, 3.5vw, 48px)' }}>{t(`pricing.items.${key}.price`)}</span>
                        <span className="text-xs text-text-muted uppercase tracking-wider">{t('pricing.currency')}</span>
                      </div>
                    </div>
                  </div>
                  <p className="text-text-secondary text-base leading-relaxed mb-8 max-w-md">{t(`pricing.items.${key}.description`)}</p>
                  <button onClick={() => scrollToBooking(key)} className="btn-primary btn-compact">{t('pricing.book')}<span className="arrow">→</span></button>
                </div>
              );
            })}
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
            ...
            <div className="lg:col-span-7 flex flex-col justify-center">
              ...
              <h2 className="text-text-primary mb-6 fade-in fade-in-1" style={{ fontSize: 'clamp(40px, 5vw, 72px)', lineHeight: '1.0' }}>{t('about.name')}</h2>
              <p className="text-text-secondary mb-10 fade-in fade-in-2">{t('about.profession')}</p>
              ...
              <div className="mt-12 fade-in fade-in-5">
      ...
      <section id="testimonials" className="section bg-primary-light">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {[
              { name: 'Елена М.', text: 'Никита — настоящий профессионал! Фотосессия прошла легко и непринуждённо. Фотографии превзошли все ожидания!', rating: 5 },
              { name: 'Александр и Мария', text: 'Заказывали love story. Результат просто потрясающий! Каждый кадр наполнен эмоциями и теплотой. Спасибо!', rating: 5 },
              { name: 'Анна К.', text: 'Очень довольна семейной фотосессией. Никита нашёл подход к детям, все были расслаблены и счастливы.', rating: 5 },
              { name: 'Дмитрий В.', text: 'Заказывал портретную съёмку для бизнеса. Всё чётко, профессионально, быстро. Рекомендую!', rating: 5 },
              { name: 'Светлана и Игорь', text: 'Свадебная фотосессия была на высоте! Никита успел запечатлеть все важные моменты. Фотографии — огонь!', rating: 5 },
              { name: 'Ольга Р.', text: 'Репортаж с дня рождения ребёнка. Никита очень тактичный и незаметный, но при этом поймал все эмоции!', rating: 5 }
            ].map((review, index) => (
      ...
      <section id="services" className="section bg-primary-light">
        <div className="container mx-auto px-4">
          <div className="mb-16 md:mb-24">
            <p className="eyebrow mb-4 fade-in">05 — {t('services.title')}</p>
          ...
          <div className="border-t border-line">
            {serviceKeys.map((key, index) => (
              <div key={key} onClick={() => scrollToBooking(key)} className="service-row group border-b border-line py-8 md:py-12 cursor-pointer fade-in" style={{ animationDelay: `${0.08 * (index + 1)}s` }}>
                <div className="grid grid-cols-12 gap-4 items-center">
                  <div className="col-span-1"><span className="eyebrow text-text-muted">0{index + 1}</span></div>
                  <div className="col-span-11 md:col-span-5">
                    <h3 className="text-text-primary group-hover:text-accent transition-colors" style={{ fontSize: 'clamp(28px, 3vw, 44px)', lineHeight: '1.1' }}>{t(`services.items.${key}.title`)}</h3>
                  </div>
                  <div className="col-span-12 md:col-span-5 md:col-start-7">
                    <p className="text-text-primary/80 text-base md:text-lg leading-relaxed max-w-lg">{t(`services.items.${key}.description`)}</p>
                  </div>
      ...
      <section id="contact" className="section bg-primary">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <p className="eyebrow mb-8 fade-in">06 — {t('contact.title')}</p>
            ...
            <p className="text-text-secondary mb-12 fade-in fade-in-2">{t('contact.description')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              <a href={`https://t.me/${CONTACTS.telegram}`} target="_blank" rel="noopener noreferrer" className="contact-card flex items-center gap-4 p-6 border border-line rounded-sm hover:border-accent/50 transition-all group fade-in fade-in-3">
                ...
                <div className="text-left"><p className="text-text-muted text-xs uppercase tracking-wider mb-1">Telegram</p><p className="text-text-primary text-lg">@{CONTACTS.telegram}</p></div>
              </a>
              <a href={`tel:${CONTACTS.phone}`} className="contact-card flex items-center gap-4 p-6 border border-line rounded-sm hover:border-accent/50 transition-all group fade-in fade-in-4">
                ...
                <div className="text-left"><p className="text-text-muted text-xs uppercase tracking-wider mb-1">{t('contact.phone')}</p><p className="text-text-primary text-lg">{CONTACTS.phoneDisplay}</p></div>
              </a>
              <a href={`https://www.instagram.com/${CONTACTS.instagram}/`} target="_blank" rel="noopener noreferrer" className="contact-card flex items-center gap-4 p-6 border border-line rounded-sm hover:border-accent/50 transition-all group fade-in fade-in-5">
                ...
                <div className="text-left"><p className="text-text-muted text-xs uppercase tracking-wider mb-1">Instagram</p><p className="text-text-primary text-lg">@{CONTACTS.instagram}</p></div>
              </a>
              <a href={`mailto:${CONTACTS.email}`} className="contact-card flex items-center gap-4 p-6 border border-line rounded-sm hover:border-accent/50 transition-all group fade-in fade-in-6">
                ...
                <div className="text-left"><p className="text-text-muted text-xs uppercase tracking-wider mb-1">{t('contact.email')}</p><p className="text-text-primary text-lg">{CONTACTS.email}</p></div>
              </a>
            </div>
          </div>
        </div>
      ...
      <section id="booking" className="section bg-primary-light">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto">
            <p className="eyebrow mb-4 fade-in text-center">07 — {t('contact.booking_title') || 'Записаться'}</p>
            ...
            {formSent ? (
              <div className="text-center p-12 border border-accent/40 rounded-sm fade-in bg-primary">
                ...
                <p className="text-text-primary text-xl">Заявка отправлена! Я свяжусь с вами в ближайшее время.</p>
              </div>
            ) : (
              <form onSubmit={handleBookingSubmit} className="space-y-6 fade-in fade-in-2 bg-primary p-8 md:p-10 rounded-sm border border-line">
                {selectedService && (
                  <div className="p-6 border border-accent/30 rounded-sm bg-accent/5 mb-6">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                      <div>
                        ...
                        <p className="text-text-primary text-xl">{t(`services.items.${selectedService}.title`)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-accent font-serif" style={{ fontSize: 'clamp(28px, 3vw, 40px)' }}>{SERVICE_PRICES[selectedService]}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {!selectedService && (
                    <div>
                      ...
                      <select value={selectedService} onChange={(e) => setSelectedService(e.target.value)} className="w-full bg-primary-light border border-line rounded-sm px-4 py-4 text-text-primary focus:border-accent focus:outline-none transition-colors">
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="text-center">
              <p className="text-text-secondary text-sm mb-2">Сохраняю моменты</p>
